...
ResultType Object::_Reverse(ExprTokenType &aResultToken, ExprTokenType *aParam[], int aParamCount)
{
	...
	if ((field1 == NULL) || (field2 == NULL)
		|| (field1 >= field2)
		|| (field2 - field1 + 1 != max_index))
		return OK; // An unexpected error, or some keys below max_index missing.

	// Swap keys: move data union and symbol, but not key:
	char *pos1 = (char *)&(*field1);
	char *pos2 = (char *)&(*field2);
	char buf[sizeof(FieldType)];
	int size1 = 2 * sizeof(void *); // Size of data union.
	int size2 = sizeof(SymbolType);
	int offset = offsetof(FieldType, symbol);
	int i;
	int size = sizeof(FieldType);
	for (i = 1; i <= max_index/2; ++i, pos1 += size, pos2 -= size)
	{
		memcpy(buf, pos1, size1);
		memcpy(pos1, pos2, size1);
		memcpy(pos2, buf, size1);
		memcpy(buf+offset, pos1+offset, size2);
		memcpy(pos1+offset, pos2+offset, size2);
		memcpy(pos2+offset, buf+offset, size2);
	}
	return OK;
}
...
ResultType Object::_Swap(ExprTokenType &aResultToken, ExprTokenType *aParam[], int aParamCount)
{
	...
	if (field1 == field2)
		return OK; // Asked to a swap key with itself.
	if ((field1 == NULL) || (field2 == NULL))
		return OK; // An unexpected error, or a key missing.
